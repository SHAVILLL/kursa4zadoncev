...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || $basePrice < 0 || $durationMinutes <= 0) {
        ...
        $stmt = $pdo->prepare('INSERT INTO services (title, description, base_price, duration_minutes, image_url, is_active) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $title,
        ...
        set_flash('success', 'Услуга успешно добавлена.');
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-success text-white"><h1 class="h4 mb-0">Новая услуга</h1></div>
                <div class="card-body">
                    <?= $message ?>
                    <form method="POST">
                        <?= csrf_input() ?>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" name="base_price" class="form-control" required min="0" step="0.01" value="<?= h($_POST['base_price'] ?? '') ?>">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Длительность, минут</label>
                                <input type="number" name="duration_minutes" class="form-control" required min="1" step="1" value="<?= h($_POST['duration_minutes'] ?? '') ?>">
                            </div>
                        </div>
                        <div class="mt-3 mb-3">
                            <label class="form-label">URL изображения</label>
                            <input type="text" name="image_url" class="form-control" value="<?= h($_POST['image_url'] ?? '') ?>">
                        </div>
